...
useEffect(() => {
  ...
  console.log('Logged in staff info:', storedStaffInfo);

  if (!storedStaffInfo) {
    console.log('No staff info found, redirecting to home.');
    window.location.href = '/';
    return;
  }

const userData = JSON.parse(storedStaffInfo);
console.log('Parsed staff data:', userData);
...
.then(res =>{
  ...
  const currentStaff = res.data.filter(s => s.id === userData.id);
  ...
  return (
    ...
    <div className='Menu'>
     <ul>
        ...
        <li>
            <button type='button' onClick={handleLogout}><LogOut size={25}/> Logout</button>
        </li>
     </ul>
    </div>
              ...
              <div className="avatar-circle">
                <span className="avatar-letter">
                  {typeof userData.name === 'string'
                    ...
                    : userData.user?.name?.charAt(0).toUpperCase() || 'S'}
                </span>
              </div>
            </div>
          </div>
